...
void HelipadDetector::init()
{
    ...
    //by now, it seems that droneArucoListMsg (3D obs) would be enough.
...
void HelipadDetector::close()
{
    return;
}

/* TODO
void HelipadDetector::readParameters()
{
    // Config files
    //
    ...
    //
    ros::param::param<std::string>("~camera_calibration_file", cameraCalibrationFile, "ardrone_front.yaml");
    ...
    // Topic names
    //
    ros::param::param<std::string>("~drone_image_topic_name", droneImageTopicName, "camera/front/image_raw");
    std::cout<<"droneImageTopicName="<<droneImageTopicName<<std::endl;
    //
...
void HelipadDetector::open(ros::NodeHandle & nIn)
{
    //Node
    ...
    if(!MyDroneArucoEye.configureArucoEye(stackPath+"configs/drone"+stringId+"/"+arucoListFile,stackPath+"configs/drone"+stringId+"/"+cameraCalibrationFile))
    {
    ...
    */
    ...
    if(!MyDroneArucoEye.setCameraInTheDrone(stackPath+"configs/drone"+stringId+"/"+droneConfigurationFile,cameraNumber))
    {
#ifdef VERBOSE_DRONE_ARUCO_EYE_ROS_MODULE
    ...
    */
    ...
    // Subscriber
    droneFrontImageSubs = n.subscribe(droneImageTopicName, 1, &HelipadDetector::droneImageCallback, this);

    //Publisher aruco 3D pose // ***It would need a special id*** TODO: Create a new 3d OBS msg only for the helipad
...
void HelipadDetector::droneImageCallback(const sensor_msgs::ImageConstPtr& msg)
{
    if(!moduleStarted)
        return;
...
int HelipadDetector::setInputImage(cv::Mat InputImageIn)
{
    flagNewImage=true;
...
//Reset
bool HelipadDetector::resetValues()
{
...
int HelipadDetector::detect(Mat source, vector < vector < Point > > Helipads, Mat &Rvec, Mat &Tvec)
 {
...
bool HelipadDetector::run()
{
    ...
    vector < vector < Point > > TheHelipad;
    Mat Rvec, Tvec;
    Tvec = Mat::zeros(3, 1, CV_32FC1);
    Rvec = Mat::zeros(3, 1, CV_32FC1);

    int nHelis = detect(InputImage,TheHelipad, Rvec, Tvec); //TODO include TheHelipad in HD.h
    ...
    if(nHelis == 1)
    {
        ...
        droneArucoListMsg.obs[0].id = 0;
        droneArucoListMsg.obs[0].x = Tvec.at<double> (0);
        droneArucoListMsg.obs[0].y = Tvec.at<double> (1);
        droneArucoListMsg.obs[0].z = Tvec.at<double> (2);
        droneArucoListMsg.obs[0].yaw   = Rvec.at<double> (0);
        droneArucoListMsg.obs[0].pitch = Rvec.at<double> (1);
        droneArucoListMsg.obs[0].roll  = Rvec.at<double> (2);
...
bool HelipadDetector::publishArucoList()
{
